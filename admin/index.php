...
/*
	$data = curlCall( "http://updates.whmcs.com/feeds/homepopup.php", array( "licensekey" => $whmcs->get_license_key(), "version" => $whmcs->get_config( "Version" ), "ssl" => (( ( isset( $_SERVER["HTTPS"] ) && $_SERVER["HTTPS"] ) && $_SERVER["HTTPS"] != "off" ) ? "1" : "0") ) );
...
/*
	if ($infotoggle[$_SESSION["adminid"]]) {
		...
		$lastupdate = curlCall( "http://updates.whmcs.com/feeds/homepopup.php", "lastupdate=1", array( "CURLOPT_TIMEOUT" => "5" ) );
...
function load_admin_home_widgets() {
	...
	while (list($key, $hook) = each($hooks[$hook_name]))
	{
		...
		if (!(in_array($widgetname, $allowedwidgets)) OR !(function_exists($hook["hook_function"])))
		{
			continue;
		}
